...
def ele_replace!(array, hash) # iterate through each ele in array. if ele is included in hash replace ele at current index with value of hash[ele]
  array.each_with_index {|ele, idx| array[idx] = hash[ele] if hash.include?(ele)} 
end
