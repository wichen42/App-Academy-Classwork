...
def is_sorted(arr)
  (0...arr.length-1).each {|i| return false if arr[i] > arr[i+1]} # generates index to compare. start at 0 and repeats until we reach index of length-1; doing more would result in overflow
  return true
end
