...
def filter_lengths(strings, length=5) # set length with default 5 if no argument is given
  strings.select {|ele| ele.length >= length} # returns arr of strings with len greather than arg(length)
end
